...
void gripper(int newPulse) { //this will be our 1600(gripper open)(in this case)
    static unsigned long timer;
    static int pulse;
    if(millis() > timer) { //timer is always 20ms greater than millis ensuring that u wait those 20ms
        if(newPulse > 0) {
            pulse = newPulse;
        }
        ...
        digitalWrite(SERVO, 0);
        timer = millis() + 20; //20 is in ms, this time is needed for the servo to process before receiving a new signal
    }
}
